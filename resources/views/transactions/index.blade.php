                        ...
                        @forelse($transactions as $item)
                            <tr>
                                ...
                                <td class="border px-6 py-4 text-center">
                                    <a href="{{route('transactions.show', $item->id)}}" class="inline-block bg-blue-500 hover:bg-blue-700 px-4 py-2 mx-2 text-white font-bold rounded">View</a>
                                    <form action="{{route('transactions.destroy', $item->id)}}" method="post" class="inline-block">
                                        {!! method_field('delete') . csrf_field() !!}
                                        <button type="submit" class="bg-red-500 hover:bg-red-700 px-4 py-2 mx-2 text-white font-bold rounded">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
